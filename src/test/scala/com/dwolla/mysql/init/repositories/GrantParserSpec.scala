...
class GrantParserSpec extends FunSuite {
  test("simple simple") {
    ...
    assertEquals(GrantParser.privileges.parse(input), Right((" ON ", NonEmptyList.of("SELECT", "DROP ROLE"))))
  }

  test("simple example") {
    val input = """GRANT SELECT, DROP ROLE ON *.*"""

    assertEquals(GrantParser.fullParser.parse(input), Right((" ON *.*", NonEmptyList.of("SELECT", "DROP ROLE"))))
  }
  ...
  test("MySQL default privileges") {
    ...
    val expectedPrivileges = NonEmptyList.of(
      "SELECT",
      "INSERT",
      "UPDATE",
      "DELETE",
