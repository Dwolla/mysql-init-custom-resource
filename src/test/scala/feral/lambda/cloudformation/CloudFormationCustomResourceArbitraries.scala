...
trait CloudFormationCustomResourceArbitraries {
  ...
  def genWrappedCloudFormationCustomResourceRequest[F[_] : Applicative, A](implicit A: Arbitrary[F[A]]): Gen[F[CloudFormationCustomResourceRequest[A]]] =
    for {
      ...
      resourceProperties <- arbitrary[F[A]]
      oldResourceProperties <- arbitrary[Option[JsonObject]]
    } yield resourceProperties.map(CloudFormationCustomResourceRequest(
      requestType,
      responseURL,
      stackId,
      requestId,
  ...
  : Gen[CloudFormationCustomResourceRequest[A]] =
    genWrappedCloudFormationCustomResourceRequest[Id, A]
